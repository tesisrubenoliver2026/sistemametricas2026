...
interface ModalCrearAsistenciaProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

const ModalCrearAsistencia = ({ isOpen, onClose, onSuccess }: ModalCrearAsistenciaProps) => {
  return (
      ...
      <Dialog as="div" className="relative z-50" onClose={onClose}>
        ...
        <div className="fixed inset-0 flex items-center justify-center p-3">
          <Dialog.Panel className="w-full max-w-[900px] max-h-[90vh] overflow-auto rounded-2xl bg-white dark:bg-gray-800 p-4 sm:p-6 text-left align-middle shadow-xl transition-all">
            <CrearAsistencia onSuccess={onSuccess} onClose={onClose} />
          </Dialog.Panel>
        </div>
      </Dialog>
    </Transition>
