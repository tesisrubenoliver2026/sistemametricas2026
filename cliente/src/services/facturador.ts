...
const isFacturadorActivo = (culminado: unknown) => {
  if (typeof culminado === 'number') return culminado === 0;
  if (typeof culminado === 'boolean') return culminado === false;
  if (typeof culminado === 'string') {
    const value = culminado.trim().toLowerCase();
    return value === 'activo' || value === '0' || value === 'false';
  }
  return false;
};

export const usuarioTieneFacturadorActivo = async () => {
  const res = await fetchAllFacturadores(1, 200, '');
  const facturadores = Array.isArray(res.data?.data) ? res.data.data : [];
  return facturadores.some((f: any) => isFacturadorActivo(f?.culminado));
};
