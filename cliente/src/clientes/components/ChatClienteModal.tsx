...
import { useTextToCliente } from '../../hooks/useTextToCliente';
import type { ChatMessage, ParsedCliente } from '../../types/chat';

interface ChatClienteModalProps {
  visible: boolean;
  onClose: () => void;
  onClienteActuado?: (data: ParsedCliente) => void;
}

const ChatClienteModal: React.FC<ChatClienteModalProps> = ({
  visible,
  onClose,
  onClienteActuado,
}) => {
  ...
  const { sendMessage, isProcessing, error, resetError } = useTextToCliente();
  ...
  const handleSubmit = async () => {
    ...
    if (response) {
      ...
      if (response.resultado && !response.esperandoRespuesta) {
        ...
        onClienteActuado?.(response.resultado);
        setShowSuccess(true);
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 px-6 py-5 flex-shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-white text-xl font-bold">Chat IA - Clientes</h2>
                <p className="text-blue-100 text-xs">Conversa naturalmente para gestionar clientes</p>
              </div>
            </div>
            <button
              onClick={handleClose}
            ...
            <div className="text-center py-8">
              <div className="bg-blue-50 rounded-xl p-6 max-w-md mx-auto">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Puedo ayudarte a crear, editar o eliminar clientes. Solo dime qué necesitas hacer.
                </p>
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-gray-700">Ejemplos:</p>
                  {[
                    'Quiero crear un nuevo cliente',
                    'Necesito editar el teléfono de un cliente',
                    'Eliminar cliente Juan Pérez',
                  ].map((example, index) => (
                    <button
                      key={index}
                      onClick={() => handleQuickCommand(example)}
                      className="block w-full text-left text-xs text-blue-700 bg-white hover:bg-blue-100 px-3 py-2 rounded-lg transition-colors border border-blue-200"
                    >
                      💡 {example}
                    </button>
                  ))}
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1">
              <textarea
                ...
                className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                rows={2}
...
export default ChatClienteModal;
