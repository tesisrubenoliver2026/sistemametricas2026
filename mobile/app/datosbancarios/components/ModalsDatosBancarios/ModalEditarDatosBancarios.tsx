// components/modals/ModalEditarDatosBancarios.tsx
...
interface ModalEditarDatosBancariosProps {
  id: number | string;
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export default function ModalEditarDatosBancarios({
  id,
  isOpen,
  onClose,
  onSuccess,
}: ModalEditarDatosBancariosProps) {
  return (
    <Modal
      visible={isOpen}
      transparent
          ...
          >
            <View className="flex-1 px-6 pt-6 pb-4">
              <ScrollView
                ...
                bounces={false}
              >
                <EditarDatosBancarios id={id} onSuccess={onSuccess} onClose={onClose} />
              </ScrollView>
            </View>
          </View>
        </KeyboardAvoidingView>
