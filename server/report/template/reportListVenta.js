export const reportListVenta = `<style>
  @media print {
    @page {
      size: A4 landscape;
      margin: 15mm;
...
<div class="resumen">
  <div class="resumen-item">
    ...
    <span>{{reporte.total_ventas}}</span>
  </div>
  <div class="resumen-item">
    <strong>Ventas Contado</strong>
    <span>{{reporte.ventas_contado}}</span>
  </div>
  <div class="resumen-item">
    <strong>Ventas Crédito</strong>
    <span>{{reporte.ventas_credito}}</span>
  </div>
  <div class="resumen-item">
    <strong>Monto Total</strong>
    <span>{{reporte.monto_total_ventas}} Gs.</span>
  </div>
  <div class="resumen-item">
  ...
  <tbody>
    {{#each reporte.ventas}}
      <tr>
        ...
        <td>{{fecha}}</td>
        <td>{{nro_factura}}</td>
        <td><strong>{{cliente_nombre}}</strong></td>
        ...
        <td class="text-center">
          <span class="badge badge-{{estado_pago}}">{{estado_pago}}</span>
        </td>
        <td class="text-right">{{total_productos}}</td>
        <td class="text-right">{{cantidad_total_productos}}</td>
        <td class="text-right">{{descuento_aplicado}} Gs.</td>
        <td class="text-right">{{ganancia_total}} Gs.</td>
...
{{else}}
<div class="sin-datos">
  <p>⚠️ No hay ventas para mostrar</p>
</div>
{{/if}}
